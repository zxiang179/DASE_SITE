...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/static/js/uploadify/uploadify.css">
</head>
<body>
<body <%@ include file="../common/skin.jsp" %>>
	...
	<div class="J_content">
		<div class="mt20 plr20">
					...
					<tbody>
						<tr>
							<td class="t_right">
								...
								<div class="fr">
									<a href="javascript:myBack()"> <i class="icon"></i><span>返回上一页</span>
									</a>
								</div>
							</td>
						</tr>
			...
			<div class="J_formTable l_form_table mgt40">
				<form action="${ctx}/cms/article/ajax/save" id="editForm"
					method="post">
					<input type="hidden" name="id" value="${article.id }" /> <input
						type="hidden" name="coverImageUrl" id="coverImageUrl"
					...
					<table>
							...
							<td>
								<div class="J_toolsBar fl">
									<div class="t_opacitySelect ml10">
										<span>请选择</span> <select id="rootColumnId" name="rootColumnId"
											onChange="selectLeafColumn()" style="min-width: 200px">
											<option value="">请选择</option>
											<c:forEach items="${rootCoulumnInfoList }" var="c">
												<option value="${c.id }"
													<c:if test="${c.id == rootColumnId }">selected="selected"</c:if>>${c.name }</option>
											</c:forEach>
										</select>
									</div>
								</div>
						...
						<tr>
							...
							<td>
								<div class="J_toolsBar fl">
									<div class="t_opacitySelect ml10">
										<span>请选择</span> <select id="leafColumnId" name="leafColumnId"
							...
							<td>
								<div class="J_toolsBar fl">
									<div class="t_check ml10">
										<label><input name="articleType" type="radio"
											value="contentType"
						...
						<tr id="contentTr">
							...
							<td>
								<div class="J_toolsBar">
									<label> <script type="text/plain" id="content"
											name="content">${article.content}</script>
									</label>
								</div>
							</td>
						</tr>
						...
						<tr id="hrefTr" style="display: none">
							...
							<td>
								<div class="J_toolsBar fl">
									<div class="t_text w600 ml10">
										<label> <input type="text" name="href"
											value="${article.href }">
										</label>
									</div>
								</div>
							...
							<td>
								<div class="J_toolsBar fl">
									<div class="t_img w100 ml10">
										<c:choose>
											<c:when
												test="${article.coverImageUrl != null && article.coverImageUrl != ''}">
												<img src="${ctx }${article.coverImageUrl}" id="attachURL"
						...
						<tr>
							<td class="l_title w200">附件：</td>
							<td>
								<div class="J_toolsBar fl">
									<div class=" w200 ml10">
										<label> <input type="file" id="attachFile" />
										</label>
									</div>
								</div>
								<div id="attatchFileURL" style="clear: both; float: left;">
								</div> <a class="abtn" style="float: left"
								href="javascript:copyToClipBoard();">复制链接</a>
							</td>
						</tr>
							...
							<td>
								<div class="J_toolsBar fl">
									<div class="t_text w200 ml10">
										<label> <input type="text" name="publisher"
						...
						<tr id="orderNoTr" style="display: none">
							...
							<td>
								<div class="J_toolsBar fl">
									<div class="t_text w200 ml10">
										<label> <input type="text" name="orderNo"
											value="${article.orderNo }">
										</label>
									</div>
								</div>
							</td>
	...
	<script type="text/javascript"
		src="${pageContext.request.contextPath}/ueditor/editor_all.js"></script>
	<script
						...
						success : function(obj) {
							...
							if (obj != null && obj.data != null) {
								...
								for (var i = 0; i < obj.data.length; i++) {
									...
									resultMsg += '<option value="'+obj.data[i].id+'" '+selectedStr+'>'
											+ obj.data[i].name + '</option>';
								}
							}
							$("#leafColumnId").append(resultMsg);
							if (articleId == null || articleId == '') {
								$("#rootColumnId").prev("span").html(
										$("#rootColumnId").find(
												"option:selected").html());
								$("#leafColumnId").prev("span").html(
										$("#leafColumnId").find(
												"option:selected").html());
							} else {
								// 会导致本方法无限触发
				...
				.uploadify(
						{
							...
							'onUploadSuccess' : function(file, data, response) {
								if (data != null) {
									port = ":8080";
									if (document.domain == "dase.ecnu.edu.cn")
										port = "";
									var attachUrl = 'http://'
											+ document.domain
											+ port
